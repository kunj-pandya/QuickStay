...
const ChechkBox = ({label, selected = false, onChange = () => { }}) => {
  return (
      ...
      <div>
        ...
        {roomsDummyData.map((room) => (
            <div key={room._id} className='flex flex-col md:flex-row items-start py-10 gap-6 border-b border-gray-300 last:pb-30 last:border-0' >
                <img onClick={() => {navigate(`/rooms/${room._id}`), scrollTo(0,0)}} src={room.images[0]} alt="hotel-img" title='View Room Details' className='max-h-65 md:w-/2 rounded-xl shadow-lg object-cover cursor-pointer' />
                ...
                <div className='md:w-1/2 flex flex-col gap-2'>
                  ...
                  <p className='texr-xl font-medium text-gray-700'>${room.pricePerNight}/night</p>
                </div>
            </div>
        ))}
      </div>
          ...
          <div className={`${openFilters ? 'h-auto' : "h-0 lg:h-auto"} overflow-hidden transition-all duration-700`}>
            <div className='px-5 pt-5'>
              ...
              {roomTypes.map((room, index) => (
                <ChechkBox key={index} label={room}/>
              ))}
            </div>
            <div className='px-5 pt-5'>
               <p className='font-medium text-gray-800 pb-2'>Price Range</p>
              {priceRanges.map((range, index) => (
                <ChechkBox key={index} label={` $ ${range}`}/>
              ))}
            </div>
